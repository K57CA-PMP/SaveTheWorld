...
void CIStartScene::addDialogs()
{
  switch (CIGameManager::getDialogState())
  {
    ...
    case 3:
      ...
      _dialog->setPosition(Point(_visibleOrigin.x + _visibleSize.width/1.5, _visibleOrigin.y + _visibleSize.height/2));
      _dialog->setFontColor(Color3B(255, 255, 255));
      _dialog->setFont("Arial", 50);
    ...
    case 4:
      _dialog = EditBox::create(Size(_visibleSize.width/1, _visibleSize.height/9), Scale9Sprite::create());
      _dialog->setPosition(Point(_visibleOrigin.x + _visibleSize.width/1.8, _visibleOrigin.y + _visibleSize.height/2));
      _dialog->setFontColor(Color3B(255, 255, 255));
      _dialog->setFont("Arial", 50);
...
{
  if (CIGameManager::getDialogState() != 4)
  {
...
void CIStartScene::nextBtnTouched(Ref* pSender)
{
  ...
  Scene* scene = CCTransitionMoveInR::create(0.5, CIStartScene::createScene());
...
{
  CIGameManager::setDialogState(CIGameManager::getDialogState() - 1);
  Scene* scene = CCTransitionFadeTR::create(0.5, CIStartScene::createScene());
  Director::getInstance()->sharedDirector()->replaceScene(scene);
}
