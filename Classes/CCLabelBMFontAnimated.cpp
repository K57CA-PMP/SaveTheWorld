...
{
    return CCLabelBMFontAnimated::create(str, fntFile, -1, cocos2d::TextHAlignment::LEFT, cocos2d::Point::ZERO);
}
...
void CCLabelBMFontAnimated::setCharScale(int index, float s){
  ...
  cocos2d::Sprite *charSprite = (cocos2d::Sprite*)getChildren().at(index);
...
void CCLabelBMFontAnimated::setAllCharsScale(float s){
  ...
  const long numChars = getChildren().size();
...
void CCLabelBMFontAnimated::animateInRevealFromLeft(float duration){
    ...
    cocos2d::Sprite *firstChar = (cocos2d::Sprite*)getChildren().at(0);
...
void CCLabelBMFontAnimated::animateInVortex(bool removeOnCompletion, bool createGhosts, float duration, int spins){
    ...
    if (createGhosts) {
        ...
        for (int i = 0; i < numGhosts; i++) {
            ...
            CCLabelBMFontAnimated *ghostLabel = CCLabelBMFontAnimated::create(getString().c_str(), getFntFile().c_str(), getContentSize().width*2, TextHAlignment::CENTER);
    ...
    for (int i = 0; i < getChildren().size(); i++) {
      ...
      Vector<FiniteTimeAction*> spinActions;
